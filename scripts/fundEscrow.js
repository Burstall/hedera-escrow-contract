...
async function main() {
	...
	if (args.length != 1) {
		console.log('Amount required, please specify amount (in hbar) as an argument');
		process.exit(1);
	}
	else {
		hbarAmt = args[0];
	...
	const hbarAmtObj = new Hbar(hbarAmt, HbarUnit.Hbar);
	console.log('Funding Escrow with:', hbarAmtObj.toString());

	const proceed = readlineSync.keyInYNStrict('Do you want to fund the escrow account?');

	if (proceed) {
		const [contractResults] = await contractExecuteFcn(contractId, 200_000, 'fundEscrow', [], hbarAmtObj);
